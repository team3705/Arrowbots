...
public:
...

	{
		...
		gripperMotor = new Jaguar (4); //drive using motor connected to servo channel 1
	...
	void Autonomous(void) {
		 ...
		 */
		 ...
		 float speed = 0.30; //CHECK-> enough speed to get to the peg in time as shoulder rises slowly            
...
//		 while (reachedEndOfLine == false) {
	...
	/*
		 ...
		 } else if ((leftSensor == 1 && middleSensor == 1 && rightSensor== 1) || (leftSensor == 1 && middleSensor == 0 && rightSensor == 1)) 
		 {
		...
		while (IsOperatorControl()) //This code will loop continuously as long it is operator control mode
		{
			...
			*/
			...
			if (leftstick->GetRawButton(4) == true) {
				shoulderMotor->Drive(-kMaxShoulderMotorSpeed, 0); //moves shoulderMotor down press 4 to movr up thr gtipprt
			...
			*/
			...
			if (leftstick->GetRawButton(6) == true) {
				gripperMotor->Set(-0.4); //opens gripper
			} else if (leftstick->GetRawButton(1) == true){
				gripperMotor->Set(0.4); //closes gripper
			}
			else {
			...
			if (rightstick->GetRawButton(3) == true) {
				...
				//MANUAL ARM CONTROL
				armMotor->Drive(-kMaxArmMotorSpeed, 0); //turn armMotor counter clockwise
			} else if (rightstick->GetRawButton(5) == true) {
				//MANUAL ARM CONTROL
				armMotor->Drive(kMaxArmMotorSpeed, 0); //turn armMotor cw
			} 
		}
	}
